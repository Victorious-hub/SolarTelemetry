            ...
            with open(f'{path_to_filename}.csv') as f:
                ...
                for r in reader:
                    ...
                    if prev_day != time.day:
                        ...
                        if wtf_image and len(wtf_image) >= 100 and len(wtf_image) < 200:
                            i = len(wtf_image)
                            s = wtf_image[i - 1][0] + 1
                            while i < 200:
                                rx = [s, 0, 0, 0, 0]
                    ...
                    if time.hour >= 8 and time.hour < 20:
                        ...
                        ald.append(r)
                    '''
                    if time.hour >= 8 and time.hour < 20 and irradiating >= 360 and irradiating <= 1500:  # and current >=0 and current <=15 and temperature >= 25 and temperature <= 400
                        ...
                        if prev_irr != irradiating:
                            if deriv_irr >= -3.8 and deriv_irr <= 3.8:  # and deriv_curr >= -0.2 and deriv_curr <= 0.2
                                # print(r)
                                counter += 1
                                val = [float(r[s]) for s in structure]
                                val[0] = index
